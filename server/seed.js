...
(async () => {
  try {
    ...
    console.log('MySQL synced for seeding');

    // Seed admin
    const existing = await Admin.findOne({ where: { email: 'admin@think.com' } });
    if (existing) {
      console.log('Admin already exists');
    } else {
      await Admin.create({ email: 'admin@think.com', password: 'admin123' });
    ...
    for (const section of sections) {
      const exists = await Content.findOne({ where: { sectionId: section.sectionId } });
      if (!exists) {
